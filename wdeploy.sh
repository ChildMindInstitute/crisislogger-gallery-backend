...
SSH_KEY_PATH="/Users/sajjadintel/dev/ssh/crisis3"
...
ssh -i $SSH_KEY_PATH $SERVER $PARAMS 'bash -i' <<-'ENDSSH'
  ...
  #git pull origin $BRANCH
	...
	/opt/cpanel/composer/bin/composer install --no-interaction --no-dev --prefer-dist --optimize-autoloader
